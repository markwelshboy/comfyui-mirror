...
# List the vars you want to persist:
VARS=(
  HF_TOKEN CIVITAI_TOKEN LORAS_IDS_TO_DOWNLOAD CHECKPOINT_IDS_TO_DOWNLOAD PUSH_SAGE_BUNDLE PUSH_CUSTOM_BUNDLE
...

{
  ...
  for k in "${VARS[@]}"; do
    v="${!k}"
    if [[ -n "$v" ]]; then
      # robust quoting for any value
      printf 'export %s=%q\n' "$k" "$v"
    fi
  done
} > "$SESSION_ENV"

# optional: a readable summary (no secrets) for logs
